...
@onready var hit_power: float = get_node("/root/Config").get_value("power", "roki_roki")
@onready var duration: float = get_node("/root/Config").get_value("duration", "roki_roki")
...
func _ready() -> void:
	...
	cooldown.wait_time = get_node("/root/Config").get_value("cooldown", "roki_roki")
	_ignore_self()
